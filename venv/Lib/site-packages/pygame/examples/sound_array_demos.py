...
original sounds. Currently it just uses hardcoded values for the
...
def make_echo(sound, samples_per_second, mydebug=True):
    """returns a sounds which is echoed of the last one."""
...
def slow_down_sound(sound, rate):
    """returns a sounds which is a slowed down version of the original.
    rate - at which the sounds should be slowed down.  eg. 0.5 would be half speed.
    """

    raise NotImplementedError()
    # grow_rate = 1 / rate
    # make it 1/rate times longer.
    # a1 = pg.sndarray.array(sounds)
...
def sound_from_pos(sound, start_pos, samples_per_second=None, inplace=1):
    """returns a sounds which begins at the start_pos.
    start_pos - in seconds from the beginning.
    samples_per_second -
    """

    # see if we want to reuse the sounds data or not.
    if inplace:
    ...
    # cut the beginning off the sounds at the start position.
...
def main():
    ...
    print(f"sounds.get_length {sound.get_length()}")
    ...
    #    #TODO: this is broken.
    #    print(("-" * 30) + "\n")
    #    print("Slow down the original sounds.")
    #    rate = 0.2
    #    slowed_sound = slow_down_sound(sounds, rate)
    #    slowed_sound.play()
    #    while pg.mixer.get_busy():
    #        pg.time.wait(200)
