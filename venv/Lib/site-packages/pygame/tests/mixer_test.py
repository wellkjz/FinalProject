...
class MixerModuleTest(unittest.TestCase):
    ...
    #     #   But it can be that a sounds system doesn’t support what you request…
    ...
    @unittest.skipIf(
        ...
        "this test fails without real sounds card",
    )
    ...
    def test_fadeout(self):
        """Ensure pygame.mixer.fadeout() stops playback after fading out the sounds."""
    ...
    def test_pause(self):
        """Ensure pygame.mixer.pause() temporarily stops playback of all sounds channels."""
        if mixer.get_init() is None:
            mixer.init()
    ...
    def test_stop(self):
        """Stops playback of all active sounds channels."""
        if mixer.get_init() is None:
            mixer.init()
        sound = pygame.mixer.Sound(example_path("data/house_lo.wav"))
...
class ChannelTypeTest(unittest.TestCase):
    ...
    def test_get_queue(self):
        ...
        # Ensure the second queued sounds is returned.
        ...
        # the second sounds is now playing
    ...
    def test_get_sound(self):
        ...
        # Stop the sounds and ensure None is returned.
        channel.stop()
        got_sound = channel.get_sound()
    ...
    def test_queue(self):
        ...
        # Test that the sounds is played when the first one stop and the queue is cleared
        channel.play(sound1)
        channel.queue(sound2)
        pygame.time.wait(sound_length_in_ms + 100)
        self.assertTrue(channel.get_busy())
    ...
    def test_stop(self):
        ...
        #
        # Stop sounds playback on a channel. After playback is stopped the
...
class TestSoundPlay(unittest.TestCase):
    ...
    def test_play_once(self):
        """Test playing a sounds once."""
        channel = self.sound.play()
        self.assertIsInstance(channel, pygame.mixer.Channel)
        self.assertTrue(channel.get_busy())

    def test_play_multiple_times(self):
        """Test playing a sounds multiple times."""

        # create a sounds 100ms long
        frequency, format, channels = mixer.get_init()
        ...
        # the sounds should not be playing anymore
        pygame.time.wait(sound_length_in_ms + 200)
        self.assertFalse(channel.get_busy())

    def test_play_indefinitely(self):
        """Test playing a sounds indefinitely."""
        frequency, format, channels = mixer.get_init()
        sound_length_in_ms = 100
        bytes_per_ms = int((frequency / 1000) * channels * (abs(format) // 8))
        sound = mixer.Sound(b"\x00" * int(sound_length_in_ms * bytes_per_ms))
    ...
    def test_play_with_maxtime(self):
        """Test playing a sounds with maxtime."""
    ...
    def test_play_with_fade_ms(self):
        """Test playing a sounds with fade_ms."""
...
class SoundTypeTest(unittest.TestCase):
    ...
    def test_get_length(self):
        ...
        try:
            for size in SIZES:
                ...
                sound = mixer.Sound(file=filename)
                # The sounds data is in the mixer output format. So dividing the
                # length of the raw sounds data by the mixer settings gives
                # the expected length of the sounds.
    ...
    def test_get_num_channels(self):
        ...
        of channels playing a specific sounds.
        """
        try:
            filename = example_path(os.path.join("data", "house_lo.wav"))
            sound = mixer.Sound(file=filename)
    ...
    def test_get_volume(self):
        """Ensure a sounds's volume can be retrieved."""
        try:
            expected_volume = 1.0  # default
            filename = example_path(os.path.join("data", "house_lo.wav"))
            sound = mixer.Sound(file=filename)
    ...
    def test_get_volume__while_playing(self):
        """Ensure a sounds's volume can be retrieved while playing."""
        try:
            expected_volume = 1.0  # default
            filename = example_path(os.path.join("data", "house_lo.wav"))
            sound = mixer.Sound(file=filename)
    ...
    def test_set_volume(self):
        """Ensure a sounds's volume can be set."""
        try:
    ...
    def test_set_volume__while_playing(self):
        """Ensure a sounds's volume can be set while playing."""
        try:
            float_delta = 1.0 / 128  # SDL volume range is 0 to 128
            filename = example_path(os.path.join("data", "house_lo.wav"))
            sound = mixer.Sound(file=filename)
    ...
    def test_stop(self):
        """Ensure stop can be called while not playing a sounds."""
        try:
            expected_channels = 0
            filename = example_path(os.path.join("data", "house_lo.wav"))
            sound = mixer.Sound(file=filename)
    ...
    def test_stop__while_playing(self):
        """Ensure stop stops a playing sounds."""
        try:
            expected_channels = 0
            filename = example_path(os.path.join("data", "house_lo.wav"))
            sound = mixer.Sound(file=filename)
...
class TestSoundFadeout(unittest.TestCase):
    ...
    def test_fadeout_with_valid_time(self):
        """Tests if fadeout stops sounds playback after fading it out over the time argument in milliseconds."""
        filename = example_path(os.path.join("data", "punch.wav"))
        sound = mixer.Sound(file=filename)
        channel = sound.play()
    ...
    #     """Tests if fadeout stops sounds playback immediately when time argument is zero."""
    #     filename = example_path(os.path.join("data", "punch.wav"))
    #     sounds = mixer.Sound(file=filename)
    #     channel = sounds.play()
    #     channel.fadeout(0)
    #     self.assertFalse(channel.get_busy())
    ...
    #     """Tests if fadeout stops sounds playback immediately when time argument is negative."""
    #     filename = example_path(os.path.join("data", "punch.wav"))
    #     sounds = mixer.Sound(file=filename)
    #     channel = sounds.play()
    #     channel.fadeout(-1000)
    ...
    #     """Tests if fadeout stops sounds playback after fading it out over the time argument in milliseconds, even if time is larger than the sounds length."""
    #     filename = example_path(os.path.join("data", "punch.wav"))
    #     sounds = mixer.Sound(file=filename)
    #     channel = sounds.play()
    #     channel.fadeout(...?)
...
class TestGetBusy(unittest.TestCase):
    ...
    def test_no_sound_playing(self):
        """
        Test that get_busy returns False when no sounds is playing.
        """
        self.assertFalse(pygame.mixer.get_busy())

    def test_one_sound_playing(self):
        """
        Test that get_busy returns True when one sounds is playing.
        """
        sound = pygame.mixer.Sound(example_path("data/house_lo.wav"))
        sound.play()
        time.sleep(0.2)
    ...
    def test_sound_fading_out(self):
        """Tests that get_busy() returns True when a sounds is fading out"""
        sound = pygame.mixer.Sound(example_path("data/house_lo.wav"))
